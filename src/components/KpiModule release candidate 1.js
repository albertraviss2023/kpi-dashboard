// KpiModule.js — Merged build (syntax-fixed)
// - OVERVIEW: enhanced (status-colored titles, banner with current/target/baseline + icon, disaggregation filters,
//              layout, mappings, medians, tiny captions, NDA logo)
// - REPORTS: enhanced version (curated per-step metrics, header-click multi-step trend,
//            cell-click single-step trend, CSV export)
...
/* ---------- status helpers for Overview visuals ---------- */
    ...
    <CardContent sx={{ pb: "14px !important", display: "flex", flexDirection: "column", flexGrow: 1 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
        <Typography
          ...
          fontWeight={800}
          sx={(t) => ({
            color: statusColor(item.status, t), // status-colored title
            display: "-webkit-box",
            WebkitLineClamp: 2,
...
/* ---------- KPI status banner (under rail) ---------- */
const KpiStatusBanner = ({ kpi, kpiId, status, value, selectedQuarter, activeDisaggDim, activeDisaggCat }) => {
      ...
      <CardContent>
        ...
        <Box sx={{ overflowX: "auto" }}>
          <table
            ...
            aria-label="Per-step curated metrics table"
          >
            <thead>
              <tr style={{ background: alpha(paletteTokens.primary, 0.08), textAlign: "left" }}>
                {headCols.map((c) => (
