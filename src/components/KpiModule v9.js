...
const ThickBar = { borderWidth: 1, borderRadius: 6, barPercentage: 0.95, categoryPercentage: 0.9, maxBarThickness: 48 };
const ValueLabels = {
  id: "valueLabels",
  afterDatasetsDraw(chart) {
    const { ctx, data, chartArea } = chart; const metas = chart.getSortedVisibleDatasetMetas?.() || [];
    ctx.save(); ctx.font = "11px Inter, system-ui"; ctx.fillStyle = chart.options.color || "#222";
    metas.forEach((m) => m.data?.forEach((el, i) => {
      const raw = data.datasets?.[m.index]?.data?.[i]; if (raw == null || isNaN(raw)) return;
      const val = Number.isInteger(raw) ? Number(raw).toLocaleString() : Number(raw).toFixed(2);
      const y = el.y - 6; if (y > chartArea.top - 6) { ctx.textAlign = "center"; ctx.fillText(val, el.x, y); }
    }));
    ctx.restore();
  }
};
...
const Sparkline = ({ series=[], isTime=false }) => {
  const chartRef = useRef(null); const canvasRef = useRef(null);
  useEffect(()=>{
    if(!canvasRef.current) return;
    const ctx=canvasRef.current.getContext("2d"); chartRef.current?.destroy?.();
    const labels=series.map(d=>d.quarter); const data=series.map(d=>d.value ?? null);
    chartRef.current=new ChartJS(ctx,{type:"line",data:{labels,datasets:[{data,borderWidth:2,fill:false,pointRadius:0,tension:.35,borderColor:isTime?tokens.bad:tokens.ok}]} ,
  ...
  },[JSON.stringify(series),isTime]);
  return <canvas ref={canvasRef} width={140} height={40} aria-label="Mini trend" role="img" style={{display:"block",width:140,height:40}}/>;
};
...
const stepsFor = (proc, quarter, selectedKpi) => {
  ...
  }).map(r=> ({...r, metricDays: (selectedKpi && medianKPIs.has(selectedKpi)) ? r.medianDays : r.avgDays,
                variance: (r.avgDays!=null && r.targetDays!=null) ? ( (selectedKpi && medianKPIs.has(selectedKpi) ? r.medianDays : r.avgDays) - r.targetDays ) : null }))
...
/* ---------------- Reusable charts ---------------- */
const BarChartBox = ({ data, title, id, height=340, valueLabels=true }) => (
  <Fullscreen title={title} id={id}>
    <Box sx={{height}}>
      <Bar data={data} options={{
        responsive:true,maintainAspectRatio:false,
        plugins:{ legend:{position:"top"}, tooltip:{callbacks:{label:(c)=>`${c.dataset.label}: ${Number(c.raw).toLocaleString()} (Quarter: ${c.label})`}} },
        scales:{ x:{ grid:{display:false} }, y:{ beginAtZero:true, grid:{ color:a("#000",.06)} } }
      }} plugins={valueLabels?[ValueLabels]:[]}/>
    </Box>
  </Fullscreen>
);
const LineChartBox = ({ data, title, id, height=340, maxPct=false }) => (
  <Fullscreen title={title} id={id}>
    <Box sx={{height}}>
      <Line data={data} options={{
        responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"top"}},
        scales:{ y:{ beginAtZero:true, max:maxPct?100:undefined, grid:{color:a("#000",.06)} }, x:{ grid:{display:false} } }
      }}/>
    </Box>
  </Fullscreen>
);
...
/* ---------------- Tiny components ---------------- */
const Donut = ({ labels, data, palette=[tokens.ok,tokens.warn,tokens.bad] }) => (
  <Doughnut data={{ labels, datasets:[{ data, backgroundColor: palette.slice(0,data.length), borderColor:"#fff", borderWidth:2, hoverOffset:8 }] }}
            options={{cutout:"70%",plugins:{legend:{display:false}},maintainAspectRatio:false,responsive:true,animation:false}}/>
);
...
const KpiCard = ({ item, selected, onOpenTrend, quarter }) => {
  ...
  return (
    <CardContent sx={{ pb:"14px !important", display:"flex", flexDirection:"column", gap:1, flexGrow:1 }}>
      ...
      <Box sx={{mt:1}}><Sparkline series={lastN(series,6)} isTime={item.isTime}/></Box>
...
const buildCountsDataset = (proc,kpiKey) => {
  const s = kpiCounts?.[proc]?.[kpiKey]; if(!Array.isArray(s)) return null;
  const labels = s.map(x=>x.quarter);
  return {
    labels,
    datasets:[
      { label:"On time", data:s.map(x=>Number.isFinite(x.numerator)?x.numerator:null), backgroundColor:a(tokens.ok,.75), borderColor:tokens.ok, stack:"counts", yAxisID:"y", ...ThickBar },
      { label:"Total",   data:s.map(x=>Number.isFinite(x.denominator)?x.denominator:null), backgroundColor:a(tokens.accent,.6), borderColor:tokens.accent, stack:"counts", yAxisID:"y", ...ThickBar }
    ]
  };
};
const genericVolumeKey = (proc,kpiId)=>{
  if(proc==="MA") return {key:"applications_completed",label:"Applications Completed", source:"qv"};
  if(proc==="CT") return kpiId==="pct_gcp_inspections_on_time" ? {key:"gcp_inspections_conducted",label:"GCP Inspections Conducted",source:"qv"} : {key:"applications_completed",label:"Applications Completed",source:"qv"};
  return {key:"inspections_conducted_total",label:"Inspections Conducted",source:"iv"};
};
const buildGenericVolumes = (proc,kpiId)=>{
  const src = genericVolumeKey(proc,kpiId); const block = src.source==="qv" ? (quarterlyVolumes[proc]||[]) : (inspectionVolumes[proc]||[]);
  const labels = block.map(d=>d.quarter);
  return { labels, datasets:[{ label:src.label, data:block.map(d=>d[src.key] ?? null), backgroundColor:a(tokens.primary,.75), borderColor:tokens.primary, yAxisID:"y", ...ThickBar }] };
};
const buildVolumesForKpi = (proc,kpiId)=>{
  // currently only CT special-case (requested vs conducted)
  if(proc==="CT" && kpiId==="pct_gcp_inspections_on_time"){
    const block = quarterlyVolumes.CT || []; const labels=block.map(d=>d.quarter);
    const pal=[{bg:a(tokens.primary,.75),border:tokens.primary},{bg:a(tokens.info,.65),border:tokens.info}];
    return { labels, datasets:[
      { label:"GCP inspections requested", data:block.map(d=>d.gcp_inspections_requested), backgroundColor:pal[1].bg, borderColor:pal[1].border, stack:"gcp", yAxisID:"y", ...ThickBar },
      { label:"GCP inspections conducted", data:block.map(d=>d.gcp_inspections_conducted), backgroundColor:pal[0].bg, borderColor:pal[0].border, stack:"gcp", yAxisID:"y", ...ThickBar }
    ]};
  }
  return null;
};
const Insights = ({ proc, selectedKpi, statusCounts, stepCounts, quarter }) => {
  const chart = selectedKpi && selectedKpi.startsWith("pct_")
    ? (buildCountsDataset(proc,selectedKpi) || buildVolumesForKpi(proc,selectedKpi) || buildGenericVolumes(proc,selectedKpi))
    : (selectedKpi ? (buildVolumesForKpi(proc,selectedKpi) || buildGenericVolumes(proc,selectedKpi)) : buildGenericVolumes(proc,selectedKpi));
  const { labels=[], datasets=[] } = chart||{};
  if (!datasets.length || datasets.every(ds=>ds.data.every(v=>v==null))) return <Typography color="text.secondary">No volume data available for {proc}</Typography>;
  const title = `Quarter ${quarter} — ${selectedKpi ? tinyKpiLabel(selectedKpi) : "volumes"}`;
  return (
    <Paper elevation={0} sx={{p:2,borderRadius:2,boxShadow:3}}>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={4}>
          <Card sx={{height:"100%",boxShadow:3,display:"flex",flexDirection:"column"}}>
            <CardContent sx={{p:2,flexGrow:1}}>
              ...
              <Box sx={{height:120,display:"flex",justifyContent:"center"}}><Donut labels={["On track","At risk","Off track"]} data={[statusCounts.success,statusCounts.warning,statusCounts.error]}/></Box>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{height:"100%",boxShadow:3,display:"flex",flexDirection:"column"}}>
            <CardContent sx={{p:2,flexGrow:1}}>
              ...
              <Box sx={{height:120,display:"flex",justifyContent:"center"}}><Donut labels={["On-track","Off-track"]} data={[stepCounts.on,stepCounts.off]} palette={[tokens.ok,tokens.bad]}/></Box>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{height:"100%",boxShadow:3,display:"flex",flexDirection:"column"}}>
            <CardContent sx={{p:2,flexGrow:1}}>
              <Typography variant="h6" fontWeight={900} align="center" color="primary">{title}</Typography>
              <BarChartBox data={{labels,datasets}} title={title} id={`ins-${proc}`}/>
            </CardContent>
          </Card>
        </Grid>
      </Grid>
...
/* ---------------- KPI details ---------------- */
const KpiDetails = ({ proc, kpiId, quarter, refEl, disDim, disCat }) => {
  ...
  return (
    <Card ref={refEl} sx={{mt:2,boxShadow:3}}>
      <CardContent>
        <Strip title={long}/>
        <LineChartBox data={line} title={`${long} — trend`} id={`kpi-${proc}-${kpiId}`} maxPct={String(kpiId).startsWith("pct_")}/>
      </CardContent>
    </Card>
  );
};
...
const StepsBar = ({ rows, quarter, useMedian }) => {
  ...
  const data = {
    labels,
    datasets:[
      { label:"Target days", data:rows.map(r=>r.targetDays), backgroundColor:a(tokens.accent,.4), borderColor:tokens.accent, ...ThickBar },
      { label: useMedian?"Median days":"Average days", data:rows.map(r=>r.metricDays), backgroundColor:rows.map(r=>r.metricDays<=r.targetDays?tokens.ok:tokens.bad), borderColor:"transparent", ...ThickBar }
    ]
  };
  return (
    <Card sx={{mt:2}}><CardContent sx={{p:2}}>
      ...
      <BarChartBox data={data} title="Process steps — actual vs target" id="steps-actual-target" height={300}/>
    </CardContent></Card>
  );
};
...
const StepTrendModal = ({ spec, onClose }) => {
  ...
  const line = { labels: s.map(d=>d.quarter),
    datasets:[
      { label:"Actual days", data:s.map(d=>d.avgDays), borderColor:tokens.primary, backgroundColor:a(tokens.primary,.2), tension:.35, fill:true },
  ...
  return (
    <Paper elevation={6} sx={{position:"fixed",zIndex:1300,left:"50%",top:"10%",transform:"translateX(-50%)",width:"min(960px,92vw)",maxHeight:"80vh",overflow:"auto",p:2,borderRadius:2}} aria-label={`Modal for ${step} details`}>
      ...
      <LineChartBox data={line} title={`${step} — Actual vs Target`} id="step-trend" height={360}/>
    </Paper>
  );
};
...
const OverviewVolumes = ({ proc, mode }) => {
  ...
  const D=(label,data,clr)=>({ label, data, backgroundColor:a(clr,.65), borderColor:clr, yAxisID:"y", ...ThickBar});
  let datasets=[];
  if(proc==="MA"){
  ...
  return <BarChartBox data={{labels,datasets}} title={`${proc} volumes`} id={`overview-${proc}`}/>;
};
...
const SingleStepTrend = ({ spec, onClose }) => {
  ...
  const line={ labels, datasets:[{ label:`${step} — ${title}`, data, borderColor:tokens.primary, backgroundColor:a(tokens.primary,.2), tension:.35, fill:true }] };
  return (
    <Paper elevation={6} sx={{position:"fixed",zIndex:1300,left:"50%",top:"10%",transform:"translateX(-50%)",width:"min(980px,94vw)",maxHeight:"80vh",overflow:"auto",p:2,borderRadius:2}}>
      ...
      <LineChartBox data={line} title={`${title} — ${step}`} id="single-step" height={360}/>
    </Paper>
  );
};
const MultiStepTrend = ({ spec, onClose }) => {
  ...
  const datasets = stepKeys.map((step,idx)=>{
    ...
    return { label:step, data, borderColor:palette[idx%palette.length], backgroundColor:"transparent", tension:.35, pointRadius:2 };
  });
  return (
    <Paper elevation={6} sx={{position:"fixed",zIndex:1300,left:"50%",top:"8%",transform:"translateX(-50%)",width:"min(1100px,96vw)",maxHeight:"84vh",overflow:"auto",p:2,borderRadius:2}}>
      ...
      <LineChartBox data={{labels,datasets}} title={`${title} — trend by step`} id="multi-step" height={420}/>
    </Paper>
  );
};
...
const KpiModuleInner = ({ themeMode, setThemeMode }) => {
  ...
  return (
    <Box sx={{width:"100%"}}>
      ...
      {tab==="Overview" && (
        <Paper elevation={0} sx={t=>({
          position:"sticky", top:84, zIndex:t.zIndex.appBar+15,
          background:t.palette.mode==="dark"?"rgba(18,18,18,.9)":"rgba(255,255,255,.96)",
          backdropFilter:"blur(4px)", borderBottom:"1px solid", borderColor:"divider", px:{xs:1,md:2}, py:1
        })}>
          <Strip
          ...
          {kpi && quarterlyData[proc][kpi]?.disaggregations && (
          ...
          <Stack spacing={2}>
            <Grid container spacing={2}>
              <Grid item xs={12} md={5}>
                <Insights proc={proc} selectedKpi={kpi}
                  statusCounts={kpis.reduce((acc,i)=>{acc[i.status]++;return acc;},{success:0,warning:0,error:0})}
                  stepCounts={stepCounts} quarter={q}/>
              </Grid>
              <Grid item xs={12} md={7}>
                <KpiDetails proc={proc} kpiId={kpi} quarter={q} refEl={detailsRef} disDim={disDim} disCat={disCat}/>
              </Grid>
            </Grid>
...
const KpiModule = () => {
  ...
  useEffect(()=>{ const isDark=themeMode==="dark";
    ChartJS.defaults.color=isDark?"#e6e6e6":"#222";
    ChartJS.defaults.borderColor=isDark?"rgba(255,255,255,.12)":"rgba(0,0,0,.12)";
  },[themeMode]);
  useEffect(()=>{ const m=window.matchMedia("(prefers-reduced-motion: reduce)"); ChartJS.defaults.animation=!m.matches; },[]);
