...
import {
  ...
  InputLabel, Grid, Card, CardContent, Divider, useTheme, Avatar,
  Fade, Chip, Stack, Badge, Modal, IconButton, useMediaQuery, Slide,
  Tooltip
} from '@mui/material';
import {
  ArrowUpward, ArrowDownward, TrendingFlat,
  Timeline, BarChart, Assignment, Science, Gavel,
  CheckCircle, Warning, Error, FilterAlt, FilterAltOff,
  Close, Brightness4, Brightness7
...
ChartJS.register(
  ...
  ChartTooltip, Legend, ArcElement, LineElement, PointElement, Filler 
);

// Color palette optimized for accessibility and consistency
const ndaColors = {
  primary: '#2E7D32', // Professional green
  secondary: '#FFB300', // Rich amber
  accent: '#2c58ddff', // Standard blue
  success: '#66BB6A',
  warning: '#FFB300',
  error: '#DC143C',
  info: '#1976D2'
...
// Short KPI names for compact displays
...
const processIcons = {
  MA: <Assignment fontSize="large" />,
  CT: <Science fontSize="large" />,
  GMP: <Gavel fontSize="large" />
};
...
const TrendIcon = ({ trend, isTimeBased }) => {
  if (isTimeBased) {
    if (trend === 'up') return <ArrowUpward color="error" fontSize="small" />;
    if (trend === 'down') return <ArrowDownward color="success" fontSize="small" />;
  } else {
    if (trend === 'up') return <ArrowUpward color="success" fontSize="small" />;
    if (trend === 'down') return <ArrowDownward color="error" fontSize="small" />;
  }
  return <TrendingFlat color="warning" fontSize="small" />;
};

// KPI Card Component
const KpiCard = ({ kpiId, kpi, value, status, trend, isTimeBased, isSelected, onClick, isFiltered, index }) => {
  ...
  const needsTooltip = shortName.split('\n').length > 3;

  const sparklineData = {
    labels: kpi.data.map(d => d.quarter),
    datasets: [{
      data: kpi.data.map(d => d.value),
      borderColor: status === 'success' ? ndaColors.success : status === 'warning' ? ndaColors.warning : ndaColors.error,
      borderWidth: 2,
      tension: 0.4,
      pointRadius: 0
    }]
      ...
      <Fade in={true} style={{ transitionDelay: `${index * 50}ms` }}>
        <Card
          ...
          sx={{
            height: '100%',
            cursor: 'pointer',
            border: isSelected ? `2px solid ${ndaColors.primary}` : 'none',
          ...
          <CardContent>
            ...
            <Box display="flex" alignItems="center" justifyContent="space-between" mt={1}>
              ...
              <Box sx={{ width: '50px', height: '30px' }}>
                <Line data={sparklineData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { display: false }, y: { display: false } } }} />
              </Box>
            </Box>
...
const ProcessStepCard = ({ stepName, stepData, quarter, onClick, darkMode }) => {
  ...
  let trend = 'flat';
  if (currentIndex > 0) {
    const currentValue = currentData.avgDays;
  ...
  return (
    <Card
      ...
      sx={{
        height: '100%',
        cursor: 'pointer',
        borderLeft: `4px solid ${status === 'success' ? ndaColors.success : status === 'warning' ? ndaColors.warning : ndaColors.error}`,
      ...
      <CardContent>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          <Box>
            ...
            <Typography variant="h6" fontWeight="bold">{currentData.avgDays} days</Typography>
          </Box>
          <Box>
            <Typography variant="body2" color="text.secondary">Target:</Typography>
            <Typography variant="h6" fontWeight="bold">{currentData.targetDays} days</Typography>
          </Box>
          <Box>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={4}>
            <Card sx={{ p: 2, height: '100%', bgcolor: alpha(ndaColors.success, 0.1) }}>
              ...
              <Typography variant="h4" fontWeight="bold" color="success">{currentData.targetDays} days</Typography>
            </Card>
          </Grid>
          <Grid item xs={12} md={4}>
        ...
        <Box sx={{ mt: 4, height: 400 }}>
          <Line
            ...
            options={{
              ...
              scales: {
                y: { beginAtZero: true, grid: { color: darkMode ? alpha('#fff', 0.1) : alpha('#000', 0.05) }, ticks: { color: darkMode ? '#fff' : ndaColors.accent } },
                x: { grid: { display: false }, ticks: { color: darkMode ? '#fff' : ndaColors.accent } }
              },
              plugins: {
                legend: { position: 'top', labels: { color: darkMode ? '#fff' : ndaColors.accent, font: { weight: 'bold' } } },
                tooltip: { backgroundColor: darkMode ? alpha('#323232', 0.95) : alpha('#fff', 0.95), titleColor: darkMode ? '#fff' : ndaColors.primary, bodyColor: darkMode ? '#fff' : ndaColors.accent }
              }
            }}
          />
        </Box>
...
const KpiModule = () => {
  ...
  const theme = useMemo(
    () => createTheme({
      ...
      typography: { fontFamily: '"Public Sans", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' },
      components: {
  ...
  const allQuarters = [...new Set(Object.values(quarterlyData.MA).flatMap(kpi => kpi.data.map(item => item.quarter)))].sort((a, b) => {
    ...
    return parseInt(qA.substring(1), 10) - parseInt(qB.substring(1), 10);
  });
  ...
  const calculations = useMemo(() => {
    ...
    const getKpiTrend = (kpiData, quarter, disaggregation, dimension) => {
      const dataSet = (disaggregation && dimension && kpiData.disaggregations?.[disaggregation]?.[dimension])
      ...
      const currentValue = dataSet[currentIndex].value;
      const prevValue = dataSet[currentIndex - 1].value;
    ...
    const kpiStatusSummary = kpiSummary.reduce((acc, curr) => {
      acc[curr.status] = (acc[curr.status] || 0) + 1;
      return acc;
    }, { success: 0, warning: 0, error: 0 });

    const trendSummary = kpiSummary.reduce((acc, curr) => {
      acc[curr.trend] = (acc[curr.trend] || 0) + 1;
      return acc;
    }, { up: 0, down: 0, flat: 0 });

    const processSummary = Object.values(processStepData[activeTab]).reduce((acc, step) => {
      const quarterData = step.data.find(item => item.quarter === selectedQuarter);
      if (!quarterData) return acc;
      const variance = quarterData.avgDays - quarterData.targetDays;
      if (variance <= 0) acc.onTime++;
      else if (variance <= quarterData.targetDays * 0.2) acc.near++;
      else acc.delayed++;
      return acc;
  ...
  const handleDisaggregationSelect = (dimension, category) => {
    setActiveDisaggregation(dimension);
    setActiveDimension(category);
  };

  const clearDisaggregation = () => {
    setActiveDisaggregation(null);
    setActiveDimension(null);
  };
  ...
  // Render Functions
  const renderHeader = () => (
    <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: '16px', width: '100%' }}>
      <Box display="flex" alignItems="center" justifyContent="space-between">
        <Box display="flex" alignItems="center">
          <Avatar src={ndaLogo} alt="NDA Logo" sx={{ width: 80, height: 80, mr: 2 }} variant="rounded" />
          <Box>
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <FormControl size="small" sx={{ minWidth: 150 }}>
            ...
            <Select value={selectedQuarter} onChange={handleQuarterChange} label="Quarter">
    ...
    if (!dataPoint) return <Typography>No data for this quarter.</Typography>;

    const isPercentage = selectedKpi.startsWith('pct_');
    const valueDisplay = isPercentage ? `${dataPoint.value}%` : dataPoint.value;
    const targetDisplay = isPercentage ? `${kpi.target}%` : kpi.target;
    ...
    return (
      <Slide direction="up" in={true}>
        <Card sx={{ p: 3, mb: 3, borderRadius: '16px', width: '100%' }}>
            ...
            <Line
              ...
              options={{
                ...
                plugins: {
                  ...
                  tooltip: {
                    backgroundColor: darkMode ? alpha('#323232', 0.95) : alpha('#fff', 0.95),
                    titleColor: darkMode ? '#fff' : ndaColors.primary,
                    bodyColor: darkMode ? '#fff' : ndaColors.accent,
      ...
      datasets: [
        {
          ...
          backgroundColor: currentQuarterData.map(step => alpha(step.variance <= 0 ? ndaColors.success : step.variance <= step.targetDays * 0.2 ? ndaColors.warning : ndaColors.error, 0.8)),
          borderColor: currentQuarterData.map(step => step.variance <= 0 ? ndaColors.success : step.variance <= step.targetDays * 0.2 ? ndaColors.warning : ndaColors.error),
          borderWidth: 1,
          borderRadius: 4,
          barPercentage: 0.45,
  ...
  <Box sx={{ mt: 4, width: '100%' }}>
    ...
    <Card sx={{ p: 2, mb: 3, borderRadius: '16px', width: '100%' }}>
        ...
        <Bar
          ...
          options={{
            ...
            scales: {
              y: { 
                ...
                grid: {
                  ...
                  tickBorderDash: [8, 8], // Dashed ticks
                  tickLength: 8 // Adjust tick length
                },
                ticks: { 
                  color: darkMode ? '#fff' : ndaColors.accent,
                  padding: 8,
                  font: {
                    weight: '500' // Medium weight for better readability
                  }
                }
              },
              x: { 
            ...
            plugins: {
              ...
              tooltip: { 
                backgroundColor: darkMode ? alpha('#323232', 0.97) : alpha('#fff', 0.97), 
                titleColor: darkMode ? '#fff' : ndaColors.primary, 
                bodyColor: darkMode ? '#fff' : ndaColors.accent,
                displayColors: true,
                boxPadding: 6,
    ...
    <Typography variant="h6" gutterBottom sx={{ color: ndaColors.accent }}>Process Step Performance</Typography>
    <TableContainer component={Paper} sx={{ borderRadius: '12px', width: '100%' }}>
      <Table>
        <TableHead sx={{ bgcolor: darkMode ? '#333' : alpha(ndaColors.primary, 0.9) }}>
          <TableRow>
            ...
            <TableCell align="right" sx={{ color: 'white', fontWeight: 'bold' }}>Target Days</TableCell>
            <TableCell align="right" sx={{ color: 'white', fontWeight: 'bold' }}>Variance</TableCell>
            <TableCell align="center" sx={{ color: 'white', fontWeight: 'bold' }}>Status Trend</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {currentQuarterData.map(({ step, avgDays, targetDays, variance, trend }) => (
            <TableRow
              key={step}
              sx={{
                '&:hover': { bgcolor: darkMode ? '#2a2a2a' : '#f5f5f5' },
                bgcolor: darkMode ? 'background.paper' : 'background.default'
              }}
            >
              <TableCell component="th" scope="row">{step}</TableCell>
              <TableCell align="right">{avgDays}</TableCell>
              <TableCell align="right">{targetDays}</TableCell>
              <TableCell align="right" sx={{
                color: variance <= 0 ? ndaColors.success :
                       variance <= targetDays * 0.2 ? ndaColors.warning :
                       ndaColors.error,
                fontWeight: 'bold'
              }}>
    ...
    return (
      <Box sx={{ mt: 4, width: '100%' }}>
        ...
        <Card sx={{
          ...
          boxShadow: darkMode ? '0 8px 32px 0 rgba(0, 0, 0, 0.3)' : '0 8px 32px 0 rgba(31, 38, 135, 0.2)',
          border: darkMode ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid rgba(255, 255, 255, 0.18)',
          borderRadius: '16px',
          width: '140%'
        }}>
          <Grid container spacing={4} alignItems="flex-start">
            ...
            <Grid item xs={12}>
              ...
              <Box sx={{ 
  height: { xs: 400, md: 600 },  // Increased height for longer bars
  width: '100%',
    ...
    options={{
      ...
      scales: {
        ...
        y: {
          ...
          // This makes the bars longer by expanding the category axis
          afterFit: function(scale) {
            scale.width = 100;  // Adjust this value to control bar length
          }
        },
      },
      plugins: { 
      ...
      barThickness: 24,  // Fixed thickness for consistent bars
      maxBarThickness: 30,  // Maximum thickness
      categoryPercentage: 0.95,  // Use most of category width
      barPercentage: 0.95,  // Use most of available space
  ...
  />
            ...
            {/* Insights and Donut Chart - Now in a single row below the bar chart */}
            ...
            <Grid item xs={12} md={6}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Box sx={{ height: 220, width: 500, position: 'relative', mr: 2 }}>
                  <Doughnut
                    data={donutData}
                    options={{
                      responsive: true,
                  ...
                  <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
                    <Typography variant="h4" sx={{ fontWeight: 'bold', color: ndaColors.primary }}>
                      {kpiStatusSummary.success}
                    </Typography>
                    <Typography variant="body1" color={darkMode ? 'text.secondary' : 'text.secondary'}>/ {calculations.kpiSummary.length}</Typography>
                ...
                <Box sx={{ mt: 2 }}>
                  {donutData.labels.map((label, index) => (
                    <Box key={label} sx={{ display: 'flex', alignItems: 'center', my: 0.5 }}>
                      <Box sx={{ width: 14, height: 14, bgcolor: donutData.datasets[0].backgroundColor[index], borderRadius: '50%', mr: 1.5 }} />
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box sx={{ width: '100%', minHeight: '100vh', p: { xs: 1, sm: 2, md: 3 }, backgroundColor: 'background.default' }}>
        {renderHeader()}
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} lg={4}>
            <Box sx={{ position: 'sticky', top: '24px', width: '100%' }}>
              {renderSummaryCharts()}
            </Box>
          </Grid>
        </Grid>
