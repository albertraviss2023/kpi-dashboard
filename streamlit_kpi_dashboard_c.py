...
.header {
  ...
  color: #fff; padding: 1.2rem 2rem; margin-bottom: 2rem;
  display: flex; align-items: center; justify-content: space-between;
  box-shadow: 0 4px 12px rgba(0,99,65,0.2); border-radius: 0 0 12px 12px;
}
.header h1 { font-size: 1.8rem; margin: 0; font-weight: 700; letter-spacing: -0.02em; }
.header .subtitle { margin: 0; opacity: 0.95; font-size: 1rem; font-weight: 400; letter-spacing: 0.01em; }
.header .version { font-size: 0.85rem; opacity: 0.8; font-weight: 500; background: rgba(255,255,255,0.1); padding: 0.4rem 0.8rem; border-radius: 20px; }
...
.kpi-card { border:1px solid $BORDER_COLOR; background:#ffffff; border-left:6px solid $NDA_GREEN; border-radius:12px; padding:1rem; box-shadow:0 2px 10px rgba(0,0,0,.05); transition: transform .12s ease, box-shadow .12s ease, border-color .12s ease; }
...
/* Builder */
.stepper { display:flex; gap:.5rem; margin-bottom:.75rem; flex-wrap:wrap; }
.step-pill { background: #fff; border:1px solid $BORDER_COLOR; padding:.35rem .6rem; border-radius:999px; font-size:.82rem; color:$TEXT_DARK; }
.step-pill.active { background:$NDA_LIGHT_GREEN; border-color:$NDA_GREEN; }
.help-tag { font-size:.78rem; color:$TEXT_LIGHT; }
...
# Map disag → group used in GMP volume charts
GMP_DISAG_TO_GROUP = {
    "On-site Domestic": "Domestic",
    "On-site Foreign": "Foreign",
    "Reliance/Joint On-site Foreign": "Reliance",
    "Reliance/Joint Desk-based Foreign": "Desk",
    "Domestic Applicant": "Domestic",
    "Foreign Direct": "Foreign",
    "Foreign Reliance": "Reliance",
    "Direct NRA": "Foreign",  # best-fit mapping for CAPA source
    "Reliance Joint": "Reliance",
}
...
# Canonical step order per process (base keys)
...
def wrap_label(text: str, max_len: int = 16) -> str:
    parts, line, count = [], [], 0
...
def _step_sort_key(process: str, step_key: str) -> Tuple[int, str]:
    base = strip_disag_suffix(step_key)
    order = STEP_ORDER.get(process, [])
    try:
        idx = order.index(base)
    except ValueError:
        idx = 10_000  # unknown to end
    # Use friendly label as tiebreaker (alphabetical)
    return (idx, friendly_step_label(step_key))

# =======================
# PROCESS STEPS BLOCK (ORDERED)
# =======================
def process_steps_block(process: str, quarter: str, processStepData: Dict[str, Any], disag_choice: str):
    ...
    # Sort keys by canonical process order
    ordered_keys = sorted(steps_dict.keys(), key=lambda k: _step_sort_key(process, k))

    rows = []
    for step_key in ordered_keys:
        step_obj = steps_dict[step_key]
        ...
        if metric is None or target is None:
            continue
        base_label = friendly_step_label(step_key)
        label = wrap_label(base_label, max_len=16)
        status = get_step_status(float(metric), float(target))
        rows.append({"_key": step_key, "step": label, "Actual": float(metric), "Target": float(target), "status": status})
    ...
    # Plot with go for custom colors per Actual bar (keep canonical order)
    fig = go.Figure()
    ...
    fig.update_layout(
        ...
        title=dict(text="Actual vs Target Days per Step (Ordered)", x=0.0, xanchor="left", font=dict(size=14)),
...
# >>> BEGIN: Volume comparison helpers <<<
...
def render_kpi_comparison(process: str, kpi_id: str, quarter: str, data: Dict[str, Any], disag_choice: str):
    ...
    # NEW: respect GMP disaggregation selection in KPI details
    if process == "GMP" and disag_choice != "All":
        grp = GMP_DISAG_TO_GROUP.get(disag_choice)
        if grp and "group" in d.columns and grp in d["group"].cat.categories:
            d = d[d["group"].astype(str) == grp]
            group_levels = [grp]
            title += f" — {disag_choice}"
...
<div class="header">
  ...
  <div class="version">v4.2 — Ordered Steps + Disagg‑aware GMP Details</div>
</div>
""",
    unsafe_allow_html=True,
)
...
def prep_analysis(
    ...
    if show_pct_change and is_time_series and analysis_type == "Trend" and len(pt) > 1:
        ...
        pct_change_df.index.name = group_by
...
def render_analysis_table_and_chart(
    ...
    if selected_chart == "line":
        ...
        if trendline and len(df_plot) > 1:
            try:
                x_s = pd.to_numeric(df_plot[x_col].astype(str).str.replace('Q','').str.replace(' ',''), errors='coerce') if group_by=="quarter" else pd.to_numeric(df_plot[x_col], errors='coerce')
                mask = x_s.notna() & pd.to_numeric(df_plot[y_col], errors='coerce').notna()
                slope, intercept, r_value, p_value, std_err = stats.linregress(x_s[mask], df_plot[y_col][mask])
                r2 = r_value**2
                annotations.append(
    ...
    fig.update_layout(
        ...
        xaxis_title=f"{group_by.replace('_', ' ').title()}",
...
def flatten_steps_for_analytics(data: Dict[str, Any]) -> pd.DataFrame:
    rows = []
    # Process steps avgDays/targetDays (from processStepData)
    for proc, steps in data.get("processStepData", {}).items():
        for step_key, obj in steps.items():
            for rec in obj.get("data", []):
                ...
                base = strip_disag_suffix(step_key)
                if "avgDays" in rec:
                    rows.append({"source":"steps","process":proc,"quarter":quarter,"year":year,"metric_name":"step_avg_days","category":base,"value":rec["avgDays"]})
                if "targetDays" in rec:
                    rows.append({"source":"steps","process":proc,"quarter":quarter,"year":year,"metric_name":"step_target_days","category":base,"value":rec["targetDays"]})
    # Bottleneck metrics (from bottleneckData)
...
if tab == "Overview":
    ...
    if st.session_state.get(FOCUS_KEY):
        ...
        if disag_choice != "All" and applied is None and process == "GMP" and has_disag_for_kpi(kpi_id, process):
            st.warning(f"No disaggregated data available for '{disag_choice}' on this KPI. Showing general view.")

        st.markdown(
            f"""
            <div style="border-radius:14px; padding:1.2rem; margin-bottom:1.5rem; background:#ffffff; border:1px solid {BORDER_COLOR}; box-shadow:0 4px 18px rgba(0,0,0,.06); border-left:10px solid {status_color(s)};">
              <div style="font-size:1.1rem; font-weight:900; color:{TEXT_DARK}; margin-bottom:.5rem;">{KPI_NAME_MAP.get(effective_kpi_id, {}).get('long', kpi_id)}</div>
              <div style="font-size:.95rem; opacity:.9;"><b>Current{curr_label} ({quarter})</b>: {curr_disp}{applied_badge} • <b>Target</b>: {pct(k.get('target')) if effective_kpi_id.startswith('pct_') else (k.get('target','—'))} • <b>Baseline</b>: {pct(k.get('baseline')) if effective_kpi_id.startswith('pct_') else (k.get('baseline','—'))} • <b>Status</b>: {status_label}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        with chart_col1:
            render_kpi_comparison(process, kpi_id, quarter, data, disag_choice)
        with chart_col2:
            kpi_trend(process, kpi_id, kpis_block, quarter, disag_choice)

        with st.expander(f"🧭 Process Steps ({process}, {quarter})" + (f" — {disag_choice}" if disag_choice != 'All' else ''), expanded=(disag_choice != "All")):
    ...
    def process_step_status_counts(process: str, quarter: str, processStepData: Dict[str, Any]) -> Dict[str, int]:
        ...
        # maintain order but counts don't need it
    ...
    with left:
        st.markdown("**KPI Status Overview (general)**")
    ...
    with right:
        st.markdown(f"**Process Steps Status — {process} ({quarter})**")
        labels = ["On track", "At risk", "Off track"]
...
# =======================
# REPORTS TAB — Self-Service + Bottlenecks (steps charts ordered)
# =======================
else:
    ...
    if view == "Quarterly Volumes & Self-Service Analytics":
        panel_open("Self-Service Analytics", icon="🧮")
        st.markdown("**Build custom views of your regulatory data.** Choose period and scope, then run an analysis.")

        df_vol = flatten_volumes(data)
        df_steps = flatten_steps_for_analytics(data)

        with st.expander("📅 Step 1: Select Period (default: latest quarter)", expanded=True):
        ...
        with st.expander("🔍 Step 2: Scope Data (default: all processes)", expanded=True):
            ...
            include_steps = st.checkbox("Include Workflow Metrics (steps, backlogs, cycle times, etc.)", value=True,
                                        help="Adds step delays, backlogs, carry-over, FPY, etc. for deeper insights.")
        ...
        if pool.empty:
            ...
            st.metric("Data Preview: Total Records", len(pool), delta=f"{len(pool['metric_name'].unique())} unique metrics")

        with st.expander("📈 Step 3: Build Analysis (defaults optimized for quick insights)", expanded=True):
            ...
            compare_by_category = st.checkbox("Breakdown by Category (e.g., Domestic vs Foreign)", value=False,
                                              help="Split by sub-groups like inspection types.")
        ...
        if not pool.empty and (selected_metric_keys or (analysis_type=="Correlation" and x_metric and y_metric)):
        ...
        # ---- Bottleneck Analysis (ordered steps) ----
        @st.cache_data(show_spinner=False)
        def reports_prepare_bottleneck_df(process: str, quarter: str, bottleneck_data: Dict[str, Any]) -> pd.DataFrame:
            ...
            if not steps_data:
                default_steps = {
                    "MA": ["application_submission_review","technical_screening","committee_assignment","committee_review","report_drafting","report_publication","capa_request","capa_evaluation"],
                    "CT": ["application_submission_review","technical_screening","committee_assignment","committee_review","inspection_scheduling","inspection_execution","report_drafting","report_publication"],
                    "GMP": ["application_submission_review","inspection_scheduling","inspection_execution","report_drafting","report_publication","capa_request","capa_evaluation"],
                }
                steps_data = {step: [] for step in default_steps.get(process, ["application_submission_review", "technical_screening"]) }

            rows = []
            for step_key, series in steps_data.items():
                step = friendly_step_label(step_key)
                base_key = strip_disag_suffix(step_key)
                qrec = next((x for x in series if x.get("quarter") == quarter), None) or {}
                random.seed(f"{process}_{quarter}_{step}")
                row = {"_key": base_key, "step": step}
            ...
            df = pd.DataFrame(rows)
            # order by canonical process order
            df["_order"] = df["_key"].apply(lambda k: STEP_ORDER.get(process, []).index(k) if k in STEP_ORDER.get(process, []) else 10_000)
            df = df.sort_values(["_order", "step"]).drop(columns=["_order"]) 
            # Fill any missing numeric values deterministically
            for c in ["cycle_time_median","ext_median_days","opening_backlog","carry_over_rate","avg_query_cycles","fpy_pct","wait_share_pct","work_to_staff_ratio","sched_median_days"]:
                if c in df.columns and df[c].isna().any():
                    np.random.seed(42)
                    df[c] = df[c].fillna(np.random.uniform(1, 10, size=df[c].isna().sum()))
            return df

        panel_open(f"Bottleneck Analysis — {process_reports} ({quarter_reports})", icon="🔬")
        ...
        st.divider()
        section_header("Detailed Metrics Table", "📋")
        if df_b.empty:
